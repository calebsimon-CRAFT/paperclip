...
| `ask_user_questions`            | Short **one-shot** structured form: a handful of typed questions asked together, each with answers/options/text. | A sequential Q&A where later questions depend on earlier answers (run a **`request_confirmation`-sequence** — see below); selecting many items from a long list; single accept/reject decisions. |
| `interview`                     | Native sequential board Q&A driven one question at a time via `/respond` + `/advance`. **Documented but not the default** — a plain board comment does **not** advance an interview turn (returns `409`; see the comment-answer gap in `references/api-reference.md`), yet the board routinely answers and steers in comments. Use only when you are certain every answer will come *in the card*. | Any planning-mode interview where the board may answer or redirect in a comment (i.e. most of them — run a **`request_confirmation`-sequence** instead); a one-shot form (`ask_user_questions`); a yes/no (`request_confirmation`); a multi-select (`request_checkbox_confirmation`). |
...
- **Sequential planning Q&A → a `request_confirmation`-sequence (recommended), not `interview`.** When you are running a **sequential** board Q&A where each next question is shaped by the previous answer (planning mode; interviewing the board across a chain of issues), ask **one question at a time** as a single `request_confirmation` card with `supersedeOnUserComment: true` and `continuationPolicy: wake_assignee`. The board can (a) accept the framing, (b) **answer free-form in a comment** — which auto-dismisses the card as `superseded_by_comment` and counts as the answer/redirect — or (c) steer; all three wake you, and you post the next card from what you just learned. Do **not** batch the questions into one `ask_user_questions` form — that throws away adapting each question to the prior answer. The native `interview` kind targets this same shape but is **not the default**: it only advances via `/respond`, so a plain board comment leaves the turn stuck at `awaiting_answer` (`409`) and forces you to abandon and reopen — and the board answers/steers in comments. See `references/api-reference.md` → **Sequential planning interviews** for both patterns and the interview comment-answer gap.
...
**Interviewing the board in planning mode.** When your plan depends on answers only the board can give and later questions depend on earlier answers, run a **`request_confirmation`-sequence**: ask **one question at a time** as a single `request_confirmation` card (`supersedeOnUserComment: true`, `continuationPolicy: wake_assignee`), then post the next card from the answer. The board can accept the framing, **answer free-form in a comment** (which dismisses the card as `superseded_by_comment` and counts as the answer/redirect), or steer — every path wakes you, so the board stays in control. Do **not** front-load the whole list into an `ask_user_questions` form. The native `interview` kind is documented but **not the default**: it only advances via `/respond`, so a plain board comment stalls the turn (`409`) and forces an abandon/reopen (see the comment-answer gap in `references/api-reference.md`). See `references/api-reference.md` → **Sequential planning interviews** for both patterns and payloads.
